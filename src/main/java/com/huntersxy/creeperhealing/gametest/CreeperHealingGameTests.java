...
/**
 * Headless GameTests for CreeperHealing (<1.21.8, legacy GameTest).
 * Run via {@code :1.21.1-neoforge:runGameTestServer}.
 */
@GameTestHolder("creeperhealing")
@PrefixGameTestTemplate(false)
public class CreeperHealingGameTests {
    ...
    @GameTest(template = "empty", timeoutTicks = 600, setupTicks = 5)
    public static void testTntHealing(GameTestHelper helper) {
        ...
        // Log manager state after explosion
        ...
        // Poll for healing completion – succeedWhen will succeed as soon as the condition holds
        helper.succeedWhen(() -> {
            for (int x = 1; x <= 3; x++) {
                for (int z = 1; z <= 3; z++) {
